        ...
        {services.map((s, i) => (
          <div key={s.slug}>
            <div className="flex items-baseline gap-4">
              ...
              <Link to={s.page || `/services/${s.slug}`} className="font-display text-xl text-moss hover:text-ochre transition-colors">
                {s.title}
              </Link>
            </div>
            <p className="mt-2 ml-9 text-sm text-ink/70 max-w-2xl">{s.shortCopy}</p>
            <Link to={s.page || `/services/${s.slug}`} className="ml-9 inline-block mt-2 text-xs font-mono text-ochre">
              Learn more →
            </Link>
